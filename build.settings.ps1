...
Task AfterBuild {
    $csc = Join-Path $PSScriptRoot "packages\Microsoft.Net.Compilers\tools\csc.exe"
    $References = @(
        "$PSScriptRoot\packages\Microsoft.PowerShell.5.ReferenceAssemblies\lib\net4\System.Management.Automation.dll",
        "$PSScriptRoot\packages\SourceMapToolkit\lib\net40\SourcemapToolkit.SourcemapParser.dll"
    ) -join ","
    &$csc /reference:$References /out:"$ModuleOutDir\lib\ScriptMap.dll" /target:library $SrcRootDir\SourceMapper.cs
